...
# +
from statistics import geometric_mean as ggmean
...
class IndicatorsAggregation(GreenGrowthStuff):
    ...
    def compute(self, indicators, average_method='arithmetic'):
        ...
        if average_method == 'geometric':
            ...
            # categories = categories.groupby('index')
            #categories = categories.exp(np.mean(np.log()))
            
            #.cumprod().T #
            categories = categories.groupby('index').gmean().T 
